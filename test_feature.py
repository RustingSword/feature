...
def define_schema():
    ...
    new_feature = schema.feature.add(type=feature.INT, name='age',
                                     validator=validator,
                                     lifecycle_stage=feature.ALPHA)
    ...
    new_feature = schema.feature.add(type=feature.INT, name='sex',
                                     validator=validator,
                                     lifecycle_stage=feature.BETA)
    new_feature.desc = "user's sex"

    # deprecated feature
    new_feature = schema.feature.add(type=feature.INT,
                                     name='deprecated_feature',
                                     lifecycle_stage=feature.DEPRECATED)
    ...
    new_feature = schema.feature.add(type=feature.STRING, name='city',
                                     validator=validator)
    ...
    new_feature = schema.feature.add(type=feature.DENSE, name='embedding',
                                     validator=validator,
                                     lifecycle_stage=feature.PRODUCTION)
    new_feature.desc = 'some embedding feature'

    # dense 2
    new_feature = schema.feature.add(type=feature.DENSE, name='random')
    new_feature.desc = 'random dense feature'

    # sparse
    new_feature = schema.feature.add(type=feature.SPARSE, name='ntags')
    new_feature.desc = 'tags of news'

    # sparse
    new_feature = schema.feature.add(type=feature.SPARSE, name='utags')
    new_feature.desc = 'tags of user'

    # float
    validator = feature.Validator(float_min=0, float_max=1)
    new_feature = schema.feature.add(type=feature.FLOAT, name='ctr',
                                     validator=validator)
    ...
    validator = feature.Validator(int_min=0, int_max=2000,
                                  phase=feature.AFTER_TRANSFORM)
    features_to_cross = sorted(['utags', 'ntags', 'intermediate'])
    new_feature = schema.feature.add(type=feature.CROSS,
    ...
    new_feature = schema.feature.add(type=feature.CROSS,
                                     ...
                                     validator=validator)
    new_feature.dependency_feature.extend(features_to_cross)
    new_feature.cross_value.cross_method = feature.COSINE_SIMILARITY
    new_feature.desc = 'another cross feature example'

    # sparse
    new_feature = schema.feature.add(type=feature.SPARSE, name='intermediate',
...
def generate_feature():
    ...
    log/online data '''
    # age
    age = feature.Feature(type=feature.INT)
    age.int_value = 50
    # sex
    sex = feature.Feature(type=feature.INT)
    sex.int_value = 2
    # dense
    tensor = feature.Feature(type=feature.DENSE)
    tensor.dense_value.value.extend([1.0, 2.0, 3.0])
    # sparse
    utags = feature.Feature(type=feature.SPARSE)
    utags.sparse_value.value.extend([10, 20, 30, 40])
    ntags = feature.Feature(type=feature.SPARSE)
    ntags.sparse_value.value.extend([1, 2, 3, 4])
    # city
    city = feature.Feature(type=feature.STRING)
    city.string_value = '北京'
    # ctr
    ctr = feature.Feature(type=feature.FLOAT)
    ctr.float_value = 1

    # intermediate
    intermediate = feature.Feature(type=feature.SPARSE)
    intermediate.sparse_value.value.extend([3, 2, 1])

    # random
    random_vec = feature.Feature(type=feature.DENSE)
...
def add_cartesian_cross_feature(sample, feat):
    ...
    for name in feat.dependency_feature:
        ...
        if current_feature.type == feature.INT:
            feature_values.append([current_feature.int_value])
        elif current_feature.type == feature.SPARSE:
            feature_values.append(current_feature.sparse_value.value)
        elif current_feature.type == feature.CROSS:
...
def add_dot_cross_feature(sample, feat):
    ...
    assert feat1.type == feature.DENSE
    assert feat2.type == feature.DENSE
    value1 = feat1.dense_value.value
    value2 = feat2.dense_value.value
    assert len(value1) == len(value2)
    if feat.cross_value.cross_method == feature.COSINE_SIMILARITY:
        norm1 = norm(value1)
        norm2 = norm(value2)
    ...
    elif feat.cross_value.cross_method == feature.DOT_PRODUCT:
...
def add_cross_feature(sample, feat, schema):
    ...
    new_feature = feature.Feature(type=feature.CROSS)
    if cross_method == feature.CARTESIAN_PRODUCT:
...
def transform(sample, schema):
    ''' transform '''
    for feat in schema.feature:
        if feat.type == feature.CROSS:
            add_cross_feature(sample, feat, schema)
        feature_in_sample = sample.feature[feat.name]
        for trans in feat.transformer:
            Transform.trans_implement(feature_in_sample, trans)


def validate(sample, schema, stage=feature.BEFORE_TRANSFORM):
    ''' validate sample against schema '''
    print('='*20)
    print('validation of stage %s' % (feature.ValidatePhase.Name(stage)))
    print('='*20)
    for feat in schema.feature:
        name = feat.name
        if feat.lifecycle_stage < CURRENT_STAGE:
            print('ignore feature %s (lifecycle stage %s)' %
                  (name, feature.LifecycleStage.Name(feat.lifecycle_stage)))
            continue
        validator = feat.validator
        if validator.phase not in (stage, feature.BEFORE_AND_AFTER_TRANSFORM):
            continue
        if name not in sample.feature:
            if feat.type == feature.CROSS or validator.allow_missing:
...
def print_final_sample(sample, schema):
    ...
    for feat in schema.feature:
        if feat.lifecycle_stage < CURRENT_STAGE:
            print('ignore feature %s (lifecycle stage %s)' %
                  (feat.name, feature.LifecycleStage.Name(feat.lifecycle_stage)))
            continue
...
def main():
    ...
    # validate(sample_demo, schema_demo, feature.BEFORE_TRANSFORM)
    # transform(sample_demo, schema_demo)
    # validate(sample_demo, schema_demo, feature.AFTER_TRANSFORM)
